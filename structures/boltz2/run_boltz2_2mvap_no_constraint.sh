#!/bin/bash
# Boltz-2 ablation: 2-MVAP cofolding WITHOUT constraints
# Protein + 2 MVAP + ATP + Mg2+, no pocket/contact constraints
# 10 samples per variant (seeds 0-9), 19 variants
# Uses GPUs 3, 7, 8, 9, 13

BOLTZ2_DIR=/nfs/lambda_stor_01/homes/rzhu/PMD/structures/boltz2
INPUT_DIR=$BOLTZ2_DIR/cofold_2mvap_no_constraint
OUTPUT_DIR=$BOLTZ2_DIR/output_2mvap_no_constraint
...
echo "$(date): Starting no_constraint ablation — 10 seeds × 19 variants on ${NUM_GPUS} GPUs" | tee $LOG
...
wait
echo "$(date): All no_constraint jobs finished." | tee -a $LOG
